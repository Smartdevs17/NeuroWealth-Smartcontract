...
fn test_deposit_withdraw_never_exceeds_total_assets() {
    ...
    assert!(withdrawn_amount <= deposit_amount + yield_amount,
        "Withdrawn amount should not exceed total contribution plus yield");

    // Edge case: Withdrawal should never be negative
    assert!(withdrawn_amount >= 0, "Withdrawn amount should never be negative");
}
...
fn test_multi_user_deposit_withdraw_invariants() {
    ...
    for (user, original_deposit) in users.iter() {
        ...
        assert!(withdrawn_amount <= original_deposit + yield_amount,
            "User cannot withdraw more than their contribution plus proportional yield");

        // Verify user state is properly reset
        assert_eq!(client.get_shares(user), 0, "User shares should be zero after full withdrawal");
        assert_eq!(client.get_balance(user), 0, "User balance should be zero after full withdrawal");
    }
    ...
    assert_eq!(total_shares_after, 0, "Total shares should be zero after all withdrawals");

    // Invariant: Total assets should be reasonable (may have tiny rounding remainder)
    assert!(total_assets_after >= 0, "Total assets should never be negative");
    assert!(total_assets_after <= total_deposited + yield_amount,
        "Total assets should not exceed total deposits plus yield");

    // Invariant: Total withdrawn should not exceed total assets
    assert!(total_withdrawn <= total_assets_before,
...
fn test_shares_asset_conversions_monotonic() {
    ...
    assert_eq!(client.get_total_shares(), 0, "Initial shares should be zero");
    assert_eq!(client.get_total_assets(), 0, "Initial assets should be zero");

    // Test bootstrap case (first deposit)
    let shares_for_10 = client.preview_deposit_to_shares(&initial_deposit);
    assert_eq!(shares_for_10, initial_deposit, "First deposit should be 1:1 shares");
    ...
    assert!(total_assets_after_yield > total_shares_after_yield,
    ...
    assert!(shares_for_15 > shares_for_10, "More assets should give more shares");
    assert!(shares_for_10 > shares_for_5, "More assets should give more shares");
    ...
    assert!(shares_back <= shares_for_10, 
    ...
    assert_eq!(share_price_1, calculated_price, 
...
fn test_zero_near_zero_rounding_edges() {
    ...
    assert!(shares_for_minimal >= 1, "Minimal amount should give at least 1 share in bootstrap");
    ...
    assert!(withdrawn <= minimal_amount + tiny_yield, 
    ...
    assert!(small_withdrawn >= 0, "Small user should not lose money due to rounding");
    assert!(big_withdrawn + small_withdrawn <= big_deposit + small_deposit,
...
fn test_share_price_consistency() {
    ...
    assert!(assets1_from_shares > deposit1, "User1 should have earned yield");
    assert_eq!(assets2_from_shares, deposit2, "User2 should get exact deposit (no yield yet)");
    ...
    assert!(final_assets1 > assets1_from_shares, "User1 should benefit from second yield");
    assert!(final_assets2 > assets2_from_shares, "User2 should benefit from second yield");
    ...
    assert!(diff <= 1, "Total assets should match sum of user assets (allowing 1 unit rounding)");
}
...
fn test_extreme_rounding_scenarios() {
    ...
    assert!(withdrawn1 <= tiny_deposit + 1, "No user should gain from rounding");
    assert!(withdrawn2 >= 0, "No user should have negative withdrawal");
    assert!(withdrawn2 <= tiny_deposit + 1, "No user should gain from rounding");
    assert!(withdrawn3 >= 0, "No user should have negative withdrawal");
    assert!(withdrawn3 <= tiny_deposit + 1, "No user should gain from rounding");

    // System should remain stable
    assert_eq!(client.get_total_shares(), 0, "All shares should be burned");
    assert!(client.get_total_assets() >= 0, "Total assets should not be negative");
    assert!(total_withdrawn <= total_deposited + tiny_yield,
...
fn test_preview_functions_match_actual_conversions() {
    ...
    assert_eq!(previewed_shares, actual_shares, "Preview should match actual shares");
    ...
    assert!(diff <= 1, "Preview should match actual withdrawal (allowing 1 unit rounding)");
}
